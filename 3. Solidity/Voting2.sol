...
// Cette version interdit au voteur de voter pour sa propre proposition : Changements ligne 21 et ligne 107
contract voting is Ownable{
    ...
    function registerProposal(string memory _description) external onlyRegisteredVoter {
        ...
        Proposal memory newProposal = Proposal(_description, 0, msg.sender);
    ...
    function vote(uint _proposalId) external onlyRegisteredVoter onlyDuringVotingSession {
        ...
        require(msg.sender != proposals[_proposalId].proposer, "Voters cannot vote for their own proposal"); // NEW!
...
// Cette fonction permet juste d'ajouter les prérequis par rapport à la structure "proposals" qui retourne la même chose.
    function getWinningProposalDetails(uint _winningProposalId) external view returns (string memory description, uint voteCount, address proposer) {
    ...
    return (winningProposal.description, winningProposal.voteCount, winningProposal.proposer);
    }
